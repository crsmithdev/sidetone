...
| "working" beside the connection state, in the app | a slow pulse while a turn or a `scripts/job` job runs, with the audio cut too. Red "stalled" means the bridge sent nothing for 15 seconds while it said it worked. It is off when the agent is idle (spec 17.11) |
